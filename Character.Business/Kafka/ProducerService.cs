...
namespace Character.Business.Kafka
{
    public class ProducerService : ProducerService<KafkaTopicsOutput>
    {
        ...
        protected override async Task OperationsAsync(CancellationToken cancellation)
        {
            ...
            {
                string message = $"del record {nameof(TransactionalOutbox)} con {nameof(TransactionalOutbox.ID)} = {item.ID}, {nameof(TransactionalOutbox.Tabella)} = '{item.Tabella}' e {nameof(TransactionalOutbox.Messaggio)} = '{item.Messaggio}'";
                ...
                try
                {
                    ...
                    {
                        nameof(CharacterDb) => KafkaTopics.Character,
